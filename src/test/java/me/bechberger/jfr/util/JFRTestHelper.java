...
public class JFRTestHelper {
    ...
    public static class RoundtripVerifier {
        ...
        /**
         * Process without modification and verify the roundtrip preserves data.
         */
        public RoundtripVerifier withNoModification() throws IOException {
            JFRTestProcessor processor = new JFRTestProcessor(tempDir);
            Path processedPath = processor.from(originalPath).withNoModification().process();
            this.processedEvents = new JFREventVerifier(processedPath).getAllEvents();
            return this;
        }

        /**
         * Process with a custom modifier.
         */
        public RoundtripVerifier withModifier(JFREventModifier modifier) throws IOException {
            JFRTestProcessor processor = new JFRTestProcessor(tempDir);
            Path processedPath = processor.from(originalPath).withModifier(modifier).process();
            this.processedEvents = new JFREventVerifier(processedPath).getAllEvents();
            return this;
        }
        ...
        public RoundtripVerifier eventCountPreserved() {
            if (originalEvents.size() != processedEvents.size()) {
                ...
                List<String> lostEventTypes = new ArrayList<>();
                List<String> preservedEventTypes = new ArrayList<>();
        ...
        public RoundtripVerifier allEventTypesPreserved() {
            ...
            assertTrue(missingTypes.isEmpty(), "All event types should be preserved. Missing: " + missingTypes);
            return this;
        }
        ...
        /**
         * Verify that a specific field value is changed (modified) in events of given type.
         */
        public RoundtripVerifier fieldChanged(String eventTypeName, String fieldName) {
            ...
            assertTrue(anyChanged, String.format("Field '%s' in '%s' should be changed in at least one event",
        ...
        public RoundtripVerifier eventsOfTypeFullyPreserved(String eventTypeName) {
            ...
            List<Executable> eventAssertions = new ArrayList<>();
            for (int i = 0; i < originalEventsOfType.size(); i++) {
                final int index = i;
        ...
        private void verifyAnnotationsEqual(List<jdk.jfr.AnnotationElement> original,
            ...
            for (int i = 0; i < original.size(); i++) {
                ...
                List<Executable> assertions = valueDescriptors.stream()
                    .map(valueDesc -> {
