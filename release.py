#!/usr/bin/env python3
"""
Bump minor version and deploy basic-jfr-processor library.
...
class VersionBumper:
    ...
    def parse_version(self, version: str) -> Tuple[int, int, int]:
        ...
        parts = version.split('.')
        if len(parts) == 2:
            # Handle 2-part versions like "0.1" as "0.1.0"
    ...
    def create_github_release(self, version: str):
        ...
        if not changelog_entry:
            changelog_entry = f"Release {version}\n\nSee [CHANGELOG.md](https://github.com/parttimenerd/basic-jfr-processor/blob/main/CHANGELOG.md) for details."
...
- `basic-jfr-processor.jar` - JAR file (requires Java 21+)
...
"""
        ...
        try:
            # Build asset paths
            jar_path = self.project_root / 'target' / 'basic-jfr-processor.jar'

            assets = []
            if jar_path.exists():
                assets.append(str(jar_path) + '#basic-jfr-processor.jar')
            else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Bump version and deploy basic-jfr-processor library',
...
Note: CHANGELOG.md must have content under [Unreleased] section before releasing.
    ...
    if args.dry_run:
        ...
        if do_github_release:
            print(f"  • gh release create v{new_version} (with CHANGELOG entry + basic-processor.jar)")
    ...
    print(f"\nArtifacts:")
    print(f"  • target/basic-jfr-processor-{new_version}.jar")
    print(f"  • target/basic-jfr-processor-{new_version}-sources.jar")
    print(f"  • target/basic-jfr-processor-{new_version}-javadoc.jar")

    if do_github_release:
        print(f"\n📦 GitHub Release:")
        print(f"  https://github.com/parttimenerd/basic-jfr-processor/releases/tag/v{new_version}")

    if do_deploy:
        print(f"\n📦 Maven Central:")
        print(f"  https://central.sonatype.com/artifact/me.bechberger/basic-jfr-processor/{new_version}")
